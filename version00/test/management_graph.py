"""
MANAGEMENT Graph Builder
Fetches live data from MANAGEMENT switch and populates Neo4j
"""
...
print("=" * 60)
print("MANAGEMENT GRAPH BUILDER")
print("=" * 60)
...
print("\n[2] Connecting to MANAGEMENT...")
device = SwitchCollector("MANAGEMENT", "192.168.56.101", 5010, creds)
device.connect()
print("[OK] Connected to MANAGEMENT")
...
print("\n[9] Creating MANAGEMENT device node...")
with driver.session() as session:
    ...
    for iface in interfaces:
        iface_id = f"MANAGEMENT:{iface['interface']}"
        session.run("""
    ...
    for trunk in trunks:
        if "port" not in trunk:
            continue
        iface_id = f"MANAGEMENT:{trunk['port']}"
        session.run("""
    ...
    for neighbor in cdp:
        if "local_interface" in neighbor and "neighbor_interface" in neighbor and "neighbor_device" in neighbor:
            local_iface_id = f"MANAGEMENT:{neighbor['local_interface']}"
...
device.disconnect()
print("\n" + "=" * 60)
print("MANAGEMENT GRAPH BUILD COMPLETE")
print("=" * 60)
