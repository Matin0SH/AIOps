"""
CORE-SW1 Graph Builder
Fetches live data from CORE-SW1 switch and populates Neo4j
"""
...
print("\n[10] Creating CORE-SW1 device node...")
with driver.session() as session:
    ...
    for trunk in trunks:
        if "port" not in trunk:
            continue
        iface_id = f"CORE-SW1:{trunk['port']}"
        session.run("""
    ...
    for neighbor in cdp:
        if 'local_interface' in neighbor and 'neighbor_interface' in neighbor and 'neighbor_device' in neighbor:
            local_iface_id = f"CORE-SW1:{neighbor['local_interface']}"
...
device.disconnect()
print("\n" + "=" * 60)
print("CORE-SW1 GRAPH BUILD COMPLETE")
print("=" * 60)
