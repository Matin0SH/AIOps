"""
ACC-SW2 Graph Builder
Fetches live data from ACC-SW2 switch and populates Neo4j
"""
...
# Connect to ACC-SW2
print("\n[2] Connecting to ACC-SW2...")
device = SwitchCollector("ACC-SW2", "192.168.56.101", 5018, creds)
device.connect()
print("[OK] Connected to ACC-SW2")
...
print("\n[9] Creating ACC-SW2 device node...")
with driver.session() as session:
    ...
    for iface in interfaces:
        iface_id = f"ACC-SW2:{iface['interface']}"
        session.run("""
    ...
    for neighbor in cdp:
        if 'local_interface' in neighbor and 'neighbor_interface' in neighbor and 'neighbor_device' in neighbor:
            local_iface_id = f"ACC-SW2:{neighbor['local_interface']}"
...
device.disconnect()
print("\n" + "=" * 60)
print("ACC-SW2 GRAPH BUILD COMPLETE")
print("=" * 60)
