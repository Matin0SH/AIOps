"""
CORE-SW2 Graph Builder
Fetches live data from CORE-SW2 switch and populates Neo4j
"""
...
# Connect to CORE-SW2
print("\n[2] Connecting to CORE-SW2...")
device = SwitchCollector("CORE-SW2", "192.168.56.101", 5014, creds)
device.connect()
print("[OK] Connected to CORE-SW2")
...
print("\n[10] Creating CORE-SW2 device node...")
with driver.session() as session:
    ...
    print(f"\n[11] Creating {len(interfaces)} interface nodes...")
    for iface in interfaces:
        iface_id = f"CORE-SW2:{iface['interface']}"
        session.run("""
    ...
    # Create VLAN nodes
    print(f"\n[12] Creating {len(vlans)} VLAN nodes...")
    for vlan in vlans:
        session.run("""
    ...
    for trunk in trunks:
        if "port" not in trunk:
            continue
        iface_id = f"CORE-SW2:{trunk['port']}"
        session.run("""
    ...
    print(f"\n[14] Creating {len(macs)} MAC address nodes...")
    for mac in macs:
        session.run("""
    ...
    print(f"\n[16] Creating {len(ospf)} OSPF relationships...")
...
device.disconnect()
print("\n" + "=" * 60)
print("CORE-SW2 GRAPH BUILD COMPLETE")
print("=" * 60)
